...
public class LandingGear : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		{	
            ...
            //Bit of a hack for Unity 5. Cache velocities of rigidbody before enable wheels
            //and then re apply as otherwise enabling wheel collider components kills all
            //velocity.
            Rigidbody aircraftRigidbody = transform.root.GetComponent<Rigidbody>();
            Vector3 linearVelocity = Vector3.zero;
            Vector3 angularVelocity = Vector3.zero;
            if (GearDown)
            {
                if (null != aircraftRigidbody)
                {
                    linearVelocity = aircraftRigidbody.velocity;
                    angularVelocity = aircraftRigidbody.angularVelocity;
                }
            }
            ...
            //Reapply velocities (Unity 5 hack).
            if (GearDown)
            {
                if (null != aircraftRigidbody)
                {
                    aircraftRigidbody.velocity = linearVelocity;
                    aircraftRigidbody.angularVelocity = angularVelocity;
                }
            }
